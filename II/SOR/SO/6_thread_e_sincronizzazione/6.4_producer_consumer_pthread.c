...
void *producer(void *ptr) {
    ...
    for (i = 1; i <= MAX; i++) {
        ...
        */
        while (buffer != 0) {
            pthread_cond_wait(&condp, &the_mutex); /*aspetta il via libera dal consumatore
      avvengono due operazioni in maniera atomica, rilascio del mutex e poi si mette in attesa della condizione
       */
        }
